...
+ 成果：实现工作流→ReAct的演进，AI搜索从单轮问答到多轮长程Agent的能力升级。1）**Agent框架构建**，原工作流有搜索词改写、相关性检验、总结几个步骤，每步用独立的Prompt请求一次LLM，执行速度慢、容错能力差，使用ReAct模式将改写、相关性、总结合并进同一次LLM请求，让LLM自主决策下一步动作，直到回复用户；2）**上下文工程-提框架稳定性**，解决首次tool call不稳定、tool call死循环等ReAct框架问题；3）**上下文工程-提搜索效果**，通过思考模式、SubAgent、渐进式披露、自我进化，提高超长上下文的指令遵循度；4）**后训练-降本提效**，SubAgent模块的LLM从开源旗舰替换为SFT后的小尺寸模型；5）**后训练-Agentic RL**，搭建与线上对齐的离线Agent环境，应用于多轮tool call训练数据的合成，使用Megatron解决超大尺寸模型的并行训练，强化学习rollout阶段引入多轮tool call提升Agent规划、反思和工具调用的能力。

**[TextViz](https://danzhuibing.github.io/TextViz/)（AI制图）**

+ 背景：多数AI制图结果为栅格图片，整体效果OK，细节处易有错误，二次加工不方便。
+ 成果：基于LLM的前端Coding能力实现AI制图。1）**Agent框架构建**，搭建ReAct闭环——基于用户粘贴内容规划布局 → 调用工具分块生成制图代码 → 前端渲染截图 → VLM评估反馈 → 调用工具修复，循环迭代直至达标；2）**上下文工程**，任务遵循度差 → 引入ToDo任务管理工具，显式拆解与追踪；自评过早收敛 → 截图评估从模型自评改为独立VLM对抗评审；上下文溢出崩溃 → 增加上下文压缩；LLM自由写代码的事实性错误 → 接入专业chart制图工具。
...
+ 成果：使用RankMixer、decoder-only Transformer重构精排模型，迈入推荐大模型范式。1）**RankMixer**，搭建rankmixer结构，特征token化 → token mixing → per-token FFN，然后借助Claude Code驱动autoresearch实验循环，寻找最优的模型结构（token数、dim、layer等）；2）**decoder-only Transformer版**，构建request-level样本，对context、sequence、item三组特征进行token化，然后基于JaggedTensor表示不定长的序列构建Transformer结构；3）**全链路模型监控Agent**，构建覆盖召回、粗排、精排、特征、融合公式、各阶段一致性的全链路模型监控体系，基于Agent发现离在线特征不一致、离在线打分不一致、离在线AUC对不齐、PCOC偏差大等潜伏问题，保障效果稳定性。

**生成式搜索（2025）**

+ 背景：搜前、搜中、搜后各环节存在多个可被LLM改进的模块，能更好理解用户需求，为用户提供更准的结果。
+ 成果：在导购、query理解、向量召回、相关性模块应用LLM。1）**导购**，使用旗舰模型得到框内词/SUG词库标注数据，SFT Qwen3-0.6B模型，一次推理得到初始化词库，例行推理得到增量词库；2）**Query理解**，收集自然语言类query，通过旗舰模型完成query改写，将改写结果存储到kv供在线流程查询；3）**召回**，对每个商品进行文本结构化描述，然后将其向量化，构建商品向量索引；在线通过自然语言query向量召回、改写词向量召回、改写词文本召回；4）**相关性**，基于200万人工标注样本微调LLM，然后用微调后的LLM修正和扩充标注，再使用标注训练4层BERT小模型支持线上服务，GSB显著提升。
