...
R是一门特立独行的语言。我接触过的编程语言有：C、Java、PHP、R、Python和Javascript。除了R，其他语言的入门标志可能是你学会了用它的`if`和`for`完成了一套处理逻辑；但对R而言，入门标志恰恰是你小心谨慎地避免在R中使用`if`和`for`来完成处理逻辑。

R需要一种特殊的学习方法。学习其他的语言好比学理科，基础最重要，工具可以现学。学习R却好像念文科，知识面很重要，只要记住哪些包哪些函数适合做什么事，对语言本身的了解并不是必须品。
...
要入门R语言，最好的学习资料是[R for Data Science](http://r4ds.had.co.nz/index.html)，书中的这张插图很好低诠释了数据科学的工作内容以及R在这些内容上能够提供的全套工具：
