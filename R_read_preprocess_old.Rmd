...
## 前言
数据分析的第一步工作是把数据集读取到工作空间，并完成一些最基本的数据操作：两个数据集的合并、数据集的排序、添加新列、保存数据集。

本文基于R自带的标准库，讲解完成上述工作内容需要用到的工具。随着R社区的发展，数据读取与处理工作已经有一些第三方库（如readr、dplyr、tidyr、data.table等）提供更便捷的工具，不过掌握这些最基本的知识，依然是用R进行数据分析很难绕过的一部分。
...
*output_var*指定要输出的列，*Sys.setenv(JAVA_HOME="$YOUR_JAVA_HOME")* 指定xlsx库需要依赖的JAVA环境变量地址，**write.xlsx()**将数据集导出为excel文件格式。xlsx库也可以用来读取excel文件，不过效率比Hadley Wickham开发的readxl库要低，所以读取excel建议使用readxl。
